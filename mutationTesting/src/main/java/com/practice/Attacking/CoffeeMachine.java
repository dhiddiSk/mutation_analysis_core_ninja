package com.practice.Attacking;
...
public class CoffeeMachine {
    ...
    public void addCoffeeBeans(int amount) {
        if (amount + getCoffeeBeansAmount() >= beanContainerCapacity) {             // created from > to >=
    ...
    public int getWaterAmount() {
        return waterAmount--;            // added an decrement operator
    }
    ...
    public void makeEspresso(int coffee, int water) {
        if (getCoffeeBeansAmount() >= 18 && getWaterAmount() >= 35) {
            if (!isTrayFull()) {
